...
CREATE TABLE medichat_group_admins (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  conversation_id UUID REFERENCES medichat_conversations(id) ON DELETE CASCADE,
  user_id UUID REFERENCES profiles(id) ON DELETE CASCADE,
  created_at TIMESTAMPTZ DEFAULT now(),
...
-- Create pinned messages table with trigger-based constraint
...
ALTER TABLE medichat_group_admins ENABLE ROW LEVEL SECURITY;
...
CREATE INDEX idx_medichat_group_admins_conversation ON medichat_group_admins(conversation_id);
CREATE INDEX idx_medichat_group_admins_user ON medichat_group_admins(user_id);
...
CREATE POLICY "Group admins can manage pinned messages"
  ON medichat_pinned_messages
  FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM medichat_group_admins
...
CREATE OR REPLACE FUNCTION get_medichat_unread_count(p_conversation_id UUID)
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
